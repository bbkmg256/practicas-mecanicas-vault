...
func _on_lanzar_hechizo() -> void:
	...
	self._hechizo_fuego.position = self._jugador.get_posision_pj() + (Vector2(20, 20) * self._jugador.get_direccion_ataque())
	...
	self._hechizo_fuego.lanzar_sonido_fx()
